...
private:
    void imageCallback(const sensor_msgs::msg::Image::SharedPtr msg) {
        ...
        double mean_intensity = (mean_scalar[0] + mean_scalar[1] + mean_scalar[2]) / 3.0;

        auto end = std::chrono::high_resolution_clock::now();
        auto processing_time = std::chrono::duration_cast<std::chrono::milliseconds>(end - start).count();

        RCLCPP_INFO(this->get_logger(), "Mean Intensity: %.2f, Processing Time: %ld ms",
                    mean_intensity, processing_time);
    }
