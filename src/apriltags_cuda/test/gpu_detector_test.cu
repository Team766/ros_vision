...
protected:
  ...
  void SetUp() override {
    // Find the package share directory for test data
    std::string package_share = ament_index_cpp::get_package_share_directory("apriltags_cuda");
    std::string test_data_dir = package_share + "/test/data/";
    // Read in the image
    bgr_img = cv::imread(test_data_dir + "colorimage.jpg", cv::IMREAD_COLOR);
    cvtColor(bgr_img, yuyv_img, COLOR_BGR2YUV_YUYV);

    bgr_img_notags =
        cv::imread(test_data_dir + "colorimage_notags.jpg", cv::IMREAD_COLOR);
