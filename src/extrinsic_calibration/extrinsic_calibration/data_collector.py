...
class DataCollectorNode(Node):
    ...
    def __init__(self):
        ...
        self.capture_length_seconds = (
            self.get_parameter("capture_length_seconds")
            .get_parameter_value()
            .double_value
        )
        self.frame_rate_hz = (
            self.get_parameter("frame_rate_hz").get_parameter_value().double_value
        )
        ...
        try:
            ...
            if not self.cameras_by_serial:
                self.get_logger().error(
                    "No cameras found! Please check camera connections and /dev/v4l/by-id/"
                )
        ...
        try:
            ...
            self.get_logger().info(
                f"Loaded camera configurations for {len(self.camera_configs)} cameras"
            )
            ...
            if missing_configs:
                self.get_logger().error(
                    f"Missing system configuration for cameras: {missing_configs}"
                )
                raise Exception(
                    f"Camera configurations not found for: {missing_configs}"
                )
    ...
    def _load_system_config(self) -> Dict[str, Any]:
        ...
        for serial_id in self.serial_ids:
            if serial_id not in camera_mounted_positions:
                raise Exception(
                    f"Camera serial {serial_id} not found in system configuration"
                )
    ...
    def _setup_cameras(self):
        ...
        for serial_id in self.serial_ids:
            ...
            if "api_preference" in camera_config:
                ...
                if api_pref_str in api_preference_map:
                    ...
                    self.get_logger().warning(
                        f"Unknown API preference: {api_pref_str}, using default"
                    )

            self.get_logger().info(
                f"Opening camera {serial_id} at video index {video_index} ({width}x{height})..."
            )
            cap = cv2.VideoCapture(video_index, api_preference)

            if not cap.isOpened():
                raise Exception(
                    f"Cannot open camera {serial_id} at video index {video_index}"
                )
            ...
            self.get_logger().info(
                f"Camera {serial_id} initialized successfully, publishing to {topic_name}"
            )
    ...
    def _collect_frames(self):
        ...
        while True:
            ...
            for cam in self.cameras:
                ...
                if not success:
                    raise Exception(
                        f'Unable to retrieve frame from camera {cam["name"]}'
                    )
                ...
                try:
                    ...
                    self.get_logger().warning(
                        f'Failed to publish image for {cam["name"]}: {e}'
                    )

                output_path = os.path.join(
                    self.output_dir, f'frame_{image_cap_idx}_{cam["name"]}.png'
                )
